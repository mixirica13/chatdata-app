...
export async function getAdInsightsTool(
  parameters: Record<string, any>,
  user: UserToken
): Promise<ToolResult> {
  // Validar parâmetros
  const params = GetAdInsightsParamsSchema.parse(parameters);

  const { ad_id, date_preset = 'last_7d', time_range, fields } = params;

  const cacheKey = `ad_insights:${ad_id}:${date_preset || JSON.stringify(time_range)}`;
  const cacheTTL = date_preset === 'today' ? 300 : 3600; // 5min ou 1h

  logger.info('Executing get_ad_insights', {
    userId: user.user_id,
    adId: ad_id,
  ...
  return withCache(
    ...
    async () => {
      const metricsFields =
        fields ||
        'ad_id,ad_name,impressions,clicks,spend,ctr,cpc,cpm,reach,frequency,actions,cost_per_action_type,inline_link_clicks,inline_link_click_ctr';

      // Construir URL
      let url = `https://graph.facebook.com/v21.0/${ad_id}/insights?fields=${metricsFields}&access_token=${user.meta_access_token}`;
      ...
      if (!response.ok) {
        throw new Error(data.error?.message || 'Failed to fetch ad insights');
      }

      if (!data.data || data.data.length === 0) {
        logger.warn('No insights data available for ad', {
          adId: ad_id,
        });
        return null;
      }
      ...
      const result = {
        ad_id: insights.ad_id,
        ad_name: insights.ad_name,
      ...
      logger.info('get_ad_insights completed', {
        userId: user.user_id,
        adId: ad_id,
        spend: result.spend,
...
export default getAdInsightsTool;
