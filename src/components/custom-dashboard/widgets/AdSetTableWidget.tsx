...
import { useAdSetInsights } from '@/hooks/useMetaApi';
...
interface AdSetTableWidgetProps {
  widget: Widget;
}

export function AdSetTableWidget({ widget }: AdSetTableWidgetProps) {
  ...
  const { data, isLoading } = useAdSetInsights(accountId || '', timeRange, !!accountId);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left">
              <th className="pb-2 font-medium text-muted-foreground">Conjunto</th>
          ...
          <tbody>
            {data.map((adset, index) => (
              <tr key={index} className="border-b border-border/50 hover:bg-muted/50">
                <td className="py-2 truncate max-w-[150px]" title={adset.adset_name}>
                  {adset.adset_name}
                </td>
                <td className="py-2 text-right">
                  {formatCurrency(parseMetricValue(adset.spend))}
                </td>
                <td className="py-2 text-right">
                  {formatNumber(parseMetricValue(adset.impressions))}
                </td>
                <td className="py-2 text-right">
                  {formatNumber(parseMetricValue(adset.clicks))}
                </td>
                <td className="py-2 text-right">
                  {parseMetricValue(adset.ctr).toFixed(2)}%
                </td>
              </tr>
            ))}
          </tbody>
